...
exports.RainAccessory = void 0;
const hysteresis_1 = require("./util/hysteresis");
class RainAccessory {
    constructor(platform, accessory, config) {
        this.platform = platform;
        this.config = config;
        this.customCharacteristics = ensureCustomCharacteristics(this.platform);
        this.metadataCharacteristics = {};
        ...
        this.hysteresis = (0, hysteresis_1.makeHysteresis)(this.platform.getHysteresisConfig());
    ...
    ensureOptionalCharacteristic(kind) {
        const char = this.customCharacteristics[kind];
        if (!char) {
            return;
        }
        const CharacteristicClass = char.CharacteristicClass;
        if (!this.switchService.testCharacteristic(CharacteristicClass)) {
            this.switchService.addCharacteristic(CharacteristicClass);
        }
        this.metadataCharacteristics[kind] = this.switchService.getCharacteristic(CharacteristicClass);
    }
    clearFault() {
        if (this.faulted) {
            this.faulted = false;
    ...
    async handleSetOn(value) {
        ...
        if (this.overrideMinutes && this.overrideMinutes > 0) {
            ...
            this.overrideUntil = Date.now() + this.overrideMinutes * 60000;
...
exports.RainAccessory = RainAccessory;
...
const ensureCustomCharacteristics = (platform) => {
    const { api } = platform;
...
let customCharacteristicRegistry = null;
